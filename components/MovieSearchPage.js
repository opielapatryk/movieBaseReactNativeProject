...
export function MovieSearchPage({ navigation }){
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        {search['Search'].filter((item)=> item.Title.toLowerCase().includes(movieTitle.toLowerCase())).map((item) => <Button onPress={() => navigation.push('Movie Details', {
            movie: item,
          })} key={item['imdbID']} title={item['Title']}/>)}
      </ScrollView>
    </View>
  );
};
